...
class MethodDispatcher : public Awesomium::JSMethodHandler {
 public:
  ...
  void Delete(Awesomium::JSObject& object, Awesomium::WebString& name) {
	  if (bound_methods_.size() == 0)
		  return;

	  auto i = bound_methods_.find(ObjectMethodKey(object.remote_id(), name));
	  bound_methods_.erase(i);
  }
  /**
